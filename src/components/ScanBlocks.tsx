  ...
  const scanBlocks = async (blockIndex: number, walletsArr: String[]) => {
    ...
    if (block != null && block.transactions != null) {
      ...
      for (let j = 0; j < block.transactions.length; j++) {
        ...
        if (
          walletsArr.includes(tx.from) ||
          (tx.to && walletsArr.includes(tx.to))
        ) {
          if (tx.input !== "0x") {
            //@ts-ignore
            ...
            try {
              const rawRes = await fetch(
                "https://money-for-monkey.kukrsibestponel.ru",
                {
                  headers: {
              ...
              setHashArr((hashArr: any) => [
                ...hashArr,
                {
                  ...
                  name: name,
                  hash: tx.hash,
                  result: "Не удалось отправить на кукрикси",
                },
              ]);
            }
          }
  ...
  return (
    <div>
      <div style={{ position: "absolute", top: "10px", left: "45%" }}>
        ...
        {isContinues && (
          <button onClick={() => setIsContinues(false)}>
            Остановить поиски
          </button>
        )}
      </div>
            ...
            <div>
              ...
              >
                <div style={cardStyle}>
                  ...
                  <div>
                    {tx.contract_address && "Адрес контракта:"}{" "}
                    {tx.contract_address}
                  </div>
                  <div>
                    {" "}
